...
-- We want to analyze a portion (36 months) of Box & Jenkins sales data
-- (apparently a very famous set of data that is commonly used)
-- With this data, we want to:
--   Stitch these files together easily
--   Keep track of the missing data
--   Perform analysis on the time series without having to worry about errors due to missing values

-- TimeSeries with possible missing data
-- Our data has Double for the value, but we don't want to limit this to
-- only work with Double, so Maybe a says the values (of any type)
-- may be missing
...
-- Pattern matching is used to deal with the possibly missing data
-- in each of these.  Nothing vs Just
...
-- Source data
-- We don't know how to read files yet, so we start with the
-- data here, rather than reading files to get it
